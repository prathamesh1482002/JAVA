...
public class BackUp {
	public static void mainmethod(File file1, File file2, String path) {
		...
		try {
			String Outputpath = path + File.separator + "Output Folder";
			File newFolder = new File(Outputpath);
			newFolder.mkdirs();
			// FileInputStream file1 = new FileInputStream("D:\\prathmesh\\JAVA Projects\\Label Request Ticket w PW - W Spine Bulk.xlsm");
			try (XSSFWorkbook workbook1 = new XSSFWorkbook(file1)) {
				XSSFSheet sheet1 = workbook1.getSheet("Label Request");
				// FileInputStream file2 = new FileInputStream("D:\\prathmesh\\JAVA Projects\\SET UP SHEET.xlsx");
					...
					try (XSSFWorkbook newWorkbook = new XSSFWorkbook()) {
						...
						if (directory.exists() && directory.isDirectory()) {
							...
							if (excelFiles != null) {
								...
								// Create header row
								Row headerRow1 = outputSheet2.createRow(0);
								...
								for (File excelFile : excelFiles) {
									...
									try (Workbook workbookInput = new XSSFWorkbook(fileInputStream)) {
										Sheet sheetip = workbookInput.getSheetAt(0);
										int rowCount = sheetip.getPhysicalNumberOfRows();

										// Write file name and row count to the output sheet
										Row dataRow = outputSheet2.createRow(rowIndex++);
						...
						// Job Name
						Row RA1 = sheet2.getRow(0); // 0-based index
						Cell CA1 = RA1.getCell(0); // B represents 1 in Apache POI
						...
						// volume /issue
						Row rowB4 = sheet1.getRow(3); // 0-based index
						Cell cellB4 = rowB4.getCell(1); // B represents 1 in Apache POI

						Row rowC4 = sheet1.getRow(3); // 0-based index
						Cell cellC4 = rowC4.getCell(2); // C represents 2 in Apache POI
						// AM
						...
						//FileOutputStream outFile2 = new FileOutputStream(Outputpath + File.separator + "output.xlsx");//CCl
						FileOutputStream outFile2 = new FileOutputStream(Outputpath + File.separator + CClvalue +"_"+CH3value+".xlsx");
	...
	private static void ReportWriter(String path) {
		// String path = "D:\\prathmesh\\JAVA Projects\\Inputs";
		String Outputpath = path + File.separator + "Output Folder" + File.separator + "output.xlsx";
